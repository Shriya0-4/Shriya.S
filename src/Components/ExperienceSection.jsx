          ...
          {experiences.map((exp, index) => (
            <div key={exp.id} className="group relative">
              {index !== experiences.length - 1 && (
                <div className="absolute left-8 top-24 w-0.5 h-full bg-gradient-to-b from-orange-300 to-stone-200 opacity-30"></div>
              )}
              <div className="relative bg-white/60 backdrop-blur-sm rounded-3xl p-8 shadow-xl hover:shadow-2xl transition-all duration-500 border border-orange-100 hover:border-orange-300">
                <div className="absolute -left-4 top-8 w-8 h-8 bg-gradient-to-br from-orange-500 to-pink-500 rounded-full shadow-lg flex items-center justify-center">
                ...
                <div className="grid lg:grid-cols-3 gap-8">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {exp.skills.map((skill) => (
                          <span
                            key={skill}
                            className="px-3 py-1 bg-stone-100 text-stone-700 rounded-full text-sm font-medium hover:bg-orange-100 hover:text-orange-700 transition-colors"
                          >
                            {skill}
                          </span>
                        ))}
                    ...
                    <div className="relative p-6 bg-gradient-to-br from-orange-50 to-stone-50 rounded-2xl border border-orange-100">
                      <div className="absolute top-2 right-2 w-16 h-16 bg-gradient-to-br from-orange-400/20 to-pink-400/20 rounded-full"></div>
                      <div className="absolute bottom-2 left-2 w-8 h-8 bg-gradient-to-br from-orange-300/30 to-pink-300/30 rounded-full"></div>
                ...
                <div className="absolute inset-0 bg-gradient-to-br from-orange-500/5 to-pink-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-3xl pointer-events-none"></div>
              </div>
            </div>
          ))}
        </div>
